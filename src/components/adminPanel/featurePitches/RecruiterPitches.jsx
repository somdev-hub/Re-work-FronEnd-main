...
const RecruiterPitches = () => {
  ...
  useEffect(() => {
    fetch(`${baseUrl}/admin/featureReq`)
      .then((res) => res.json())
      .then((data) => {
        setIsLoading(false)
        setData(data);
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody>
                      {pitchArr
                        .filter((value) => {
                          return (
                            query.toLowerCase() === "" ? value : value?.ApplicationFrom?.fullName.toLowerCase().includes(query) ||
...
export default RecruiterPitches;

const MyFunction = ({ data, focusUpdate }) => {
  const [modelOpened, setModelOpened] = useState(false);
  let a = (id) => {
    fetch(`${baseUrl}/admin/feature/${id}`, { method: "PUT" })
      .then((res) => res.json())
      .then((data) => {
        focusUpdate();
      });
