...
const ApplicationsRecruiterSubmissionsViewJobs = () => {
  ...
  useEffect(() => {
    ...
    location.state.jobPostedId.forEach((element) => {
      if (element.status === "Admin-Approve") {
        arr.push(element);
      }
    });
    setJobs(arr);
  ...
  //       let url = `${baseUrl}/users/getImage/${company.companyLogo.filename}`
  //       setLogo(url)
  //     }
  //   }
  // }, [])
...
export default ApplicationsRecruiterSubmissionsViewJobs;
  ...
  return (
    <tr className="border-b" key={value._id}>
      ...
      <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap flex gap-5">
        <Link
          to={`/admin/recruitersubmissions/${value._id}/candidates`}
          state={value}
        >
