...
const CMSPrivacyPolicy = () => {
  ...
  const getPrivacyPolicy = async () => {
    try {
      ...
      setContent(jsonData.posts.privacyPolicy);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const postPrivacyPolicy = useCallback(async (data) => {
    try {
      const result = await fetch(`${baseUrl}/admin/updatePrivacyPolicy`, {
        method: 'put',
        ...
        isLoading ? <div className="flex justify-center items-center h-[60vh]">
          ...
          <div className="border-2 rounded-lg shadow-md">
            <div className="flex justify-between p-5 border-b-2">
              <h6 className="font-semibold text-xl">Privacy Policy</h6>
            </div>
