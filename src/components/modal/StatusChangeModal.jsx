...
const StatusChangeModal = ({ modelOpened, setModelOpened, data }) => {
  ...
  const statusChangeHandler = useCallback(async (userId, status) => {
    try {
      // let userId = data._id ;
      let token = localStorage.getItem('token');
      const result = await fetch(
        `${baseUrl}/payment/creditesStatusPut/${userId}`,
        {
          method: "put",
          headers: {
      ...
      <div className="md:px-10 lg:p-20">
        ...
        <div className="flex justify-center gap-10 text-white">
          ...
          <button
            ...
            onClick={(e) => {
              statusChangeHandler(data._id, data.status);
            }}
          >
            Confirm
          </button>
